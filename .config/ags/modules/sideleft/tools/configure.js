...
import {
  ...
  initGowall,
  initGowallIndex,
  initTransparencyModeVal,
} from "../../indicators/colorscheme.js";

// HyprlandToggle and HyprlandSpinButton definitions
      ...
      children: [
        ConfigSection({
          ...
          children: [
            ...
            ConfigSeparator(),
            ConfigSection({
              name: getString("Modes"),
              children: [
                ConfigToggle({
                  icon: "dock",
                  name: getString("Vertical Dock"),
                  desc: getString("Switch Dock Mode"),
                  initValue: dockMode.value,
                  onChange: (_, newValue) => {
                    dockMode.value = !!newValue;
                  },
                  setup: (self) => {
                    self.hook(dockMode, (self) => {
                      self.enabled.value = dockMode.value;
                    });
                  },
                }),
                ConfigToggle({
                  icon: "rounded_corner",
                  name: getString("Widget Corner"),
                  desc: getString("Add Cairo Widget Corners"),
                  initValue: useCorners.value,
                  onChange: (_, newValue) => {
                    useCorners.value = !!newValue;
                  },
                  setup: (self) => {
                    self.hook(useCorners, (self) => {
                      self.enabled.value = useCorners.value;
                    });
                  },
                }),
              ],
