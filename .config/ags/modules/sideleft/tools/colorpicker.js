...
export default () => {
        ...
        attribute: {
          update: (self) => {
            ...
            self.setCss(`background: 
                    ...
                    linear-gradient(to right, #ffffff, ${hslToHex(
                      selectedColor.hue,
                      100,
                      50
                    )});
        ...
        attribute: {
          update: (self) => {
            ...
            self.setCss(`
                        margin-left: ${(13.636 * selectedColor.xAxis) / 100}rem;
                        margin-top: ${
                          (13.636 * (100 - selectedColor.yAxis)) / 100
                        }rem;
                    `); // Why 13.636rem? see class name in stylesheet
          },
        },
        ...
        children: [
          Box({
            ...
            css: `
                    ...
                    border-color: ${
                      shouldUseBlackColor() ? colorBlack : colorWhite
                    };
                `,
            attribute: {
              update: (self) => {
  ...
  const resultHsl = ResultBox({
    ...
    updateCallback: (self, id) => {
      if (id && self.attribute.id === id) return;
      self.text = `${selectedColor.hue},${selectedColor.xAxis}%,${Math.round(
        selectedColor.yAxis / (1 + selectedColor.xAxis / 100)
      )}%`;
    },
    copyCallback: () =>
      Utils.execAsync([
        "wl-copy",
        `hsl(${selectedColor.hue},${selectedColor.xAxis}%,${Math.round(
          selectedColor.yAxis / (1 + selectedColor.xAxis / 100)
        )}%)`,
      ]),
  });
  ...
  const result = Box({
    ...
    hexpand: true,
    vertical: true,
    children: [resultColorBox, resultHex, resultRgb, resultHsl, applybutton],
  });
  return SidebarModule({
    icon: TablerIcon("eb61", "norm"),
