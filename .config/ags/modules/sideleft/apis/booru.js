        ...
        const imageBox = Box({
            ...
            attribute: {
                'update': (self, data, force = false) => {
                    ...
                    const widgetWidth = 12.273 * 16; // Convert rem to px (1rem = 16px by default)
                ...
                children: [
                    ...
                    ImageAction({
                        ...
                        action: (self) => {
                            ...
                            const saveCommand = `mkdir -p "$(xdg-user-dir PICTURES)/homework/${data.is_nsfw ? '🌶️/' : ''}${userOptions.asyncGet().sidebar.image.saveInFolderByTags ? tagDirectory : ''}" && curl -L -o "$(xdg-user-dir PICTURES)/homework/${data.is_nsfw ? '🌶️/' : ''}${userOptions.asyncGet().sidebar.image.saveInFolderByTags ? (tagDirectory + '/') : ''}${fileName}" '${data.file_url}'`;
                            print(saveCommand)
