...
case $SKIP_CONFIG_FILES in
	...
	*)
		...
		for i in $(find "$base/dotfiles/.config/" -mindepth 1 -maxdepth 1 ! -name 'ags' ! -name 'fish' ! -name 'hypr' -type d -exec basename {} \;); do
			config_dir=".config/$i" # Construct the full path relative to dotfiles root
			echo "[$0]: Found target config directory: $config_dir"
			if [ -d "$base/dotfiles/$config_dir" ]; then
				v rsync -av --delete "$base/dotfiles/$config_dir/" "$XDG_CONFIG_HOME/$i/"
			else
		...
		for i in $(find "$base/dotfiles/.config/" -maxdepth 1 ! -name 'ags' ! -name 'fish' ! -name 'hypr' -type f -exec basename {} \;); do
			config_file=".config/$i" # Construct the full path relative to dotfiles root
			echo "[$0]: Found target config file: $config_file"
			if [ -f "$base/dotfiles/$config_file" ]; then
				v rsync -av "$base/dotfiles/$config_file" "$XDG_CONFIG_HOME/$i"
			else
...
case $SKIP_FISH in
	...
	*)
		...
		v rsync -av --delete "$base/dotfiles/.config/fish/" "$XDG_CONFIG_HOME/fish/"
		;;
esac
...
case $SKIP_AGS in
	...
	*)
		...
		rsync_ags_source_dir="$base/dotfiles/.config/ags" # Removed "local" here

		# 1. Check for existence of "ags" folder at dotfiles/.config/ags
		...
		######################################################################
		# Part 2: Root ".ags" copy to ~/.ags (using rsync -av --delete - for synchronization of dotfiles/.ags to ~/.ags)
		case $SKIP_ROOT_DOT_AGS_COPY in
			...
			*)
				...
				source_dot_ags_dir="$base/dotfiles/.ags" # Path to dotfiles root /.ags # Removed "local" here
				dest_ags_dir="$HOME/.ags" # Removed "local" here

				# 1. Check for existence of source ".ags" folder in dotfiles/.ags
...
case $SKIP_HYPRLAND in
	...
	*)
		...
		v rsync -av --delete --exclude '/custom' --exclude '/hyprland.conf' "$base/dotfiles/.config/hypr/" "$XDG_CONFIG_HOME/hypr/"

		t="$XDG_CONFIG_HOME/hypr/hyprland.conf"
		if [ -f $t ]; then
			echo -e "\e[34m[$0]: \"$t\" already exists.\e[0m"
			# Source is now back to .config/hypr, cp source path corrected:
			v cp -f "$base/dotfiles/.config/hypr/hyprland.conf" "$t.new"
			existed_hypr_conf=y
		else
			echo -e "\e[33m[$0]: \"$t\" does not exist yet.\e[0m"
			# Source is now back to .config/hypr, cp source path corrected:
			v cp -f "$base/dotfiles/.config/hypr/hyprland.conf" "$t"
			existed_hypr_conf=n
		fi
		...
		if [ -d $t ]; then
			...
			echo -e "\e[33m[$0]: \"$t\" does not exist yet.\e[0m"
			# Source is now back to .config/hypr, rsync source path corrected:
			v rsync -av --delete "$base/dotfiles/.config/hypr/custom/" "$t/"
		fi
		;;
esac
...
case $SKIP_ROFI in # Assuming you have a SKIP_ROFI variable
	...
	*)
		...
		v rsync -av --delete "$base/dotfiles/.config/rofi/" "$HOME/.config/rofi/" # Target is now $HOME for ~/.config
		;;
esac
...
v rsync -av --delete "$base/dotfiles/.local/bin/" "$XDG_BIN_HOME"
