...
const CricBtn = (icon, clicked, props = {}) => Button({
  child: MaterialIcon(icon, 'large', { className: "bar-icon txt-large icon-material onSurfaceVariant" }),
  vpack: 'center',
  onClicked: () => clicked,
  ...props
});
...
export const NotchBar = Widget.CenterBox({
  ...
  startWidget:
    Widget.Box({
      ...
      children: [
        Battery(),
        Widget.Box({ child: await FocusOptionalWorkspaces(), className: "notch-padding-h bar-util-btn2 " }),
      ],
    }),
  centerWidget:
    Widget.Box({
  ...
  endWidget:
    Widget.Box({
      ...
      spacing: 10,
      children: [
        Widget.Box({ child: Clock(), className: "notch-padding-h bar-util-btn2 " }),
        Widget.Box({ child: StatusIcons(), className: "notch-padding-h bar-util-btn2 " }),
